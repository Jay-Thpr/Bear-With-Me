...
import { Mic, MicOff, Video as VideoIcon, VideoOff, Pause, Play, MessageSquare, X, CheckCircle, Info, PlaySquare, TriangleAlert } from "lucide-react";
...
function LiveCoachingContent() {
  ...
  const handleToolCall = useCallback(async (calls: FunctionCall[]) => {
    for (const call of calls) {
      switch (call.name) {
        case "log_observation": {
          ...
          // POST to session log API (for post-session summary)
    ...
    // Always respond to tool calls so Gemini doesn't stall
  ...
  useEffect(() => {
    ...
    async function connect() {
      ...
      try {
        ...
        if (cancelled) return;
        console.error("[live] Token fetch failed:", e.message);
      ...
      client.connect(accessToken, liveModel, systemPrompt, COACH_FUNCTION_DECLARATIONS, {
        onSetupComplete: () => {
          ...
          console.log("[live] Gemini Live session ready");
          setCoachPhase("live");
          ...
          void (async () => {
            try {
              ...
              console.error("[live] Mic failed:", e.message);
            }
          })();

          // Start 1 FPS video loop
        ...
        onError: (msg) => {
          console.error("[live] Gemini error:", msg);
          setCoachError(msg);
          setCoachPhase("error");
          void stopMedia();
        },
